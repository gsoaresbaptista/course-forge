...
class SchemdrawProcessor(SVGProcessorBase):
    ...
    def execute(self, node: ContentNode, content: str) -> str:
        ...
        for match in matches:
            ...
            try:
                # Use class name to avoid collisions between SchemdrawProcessor and BlockDiagramProcessor
                proc_name = self.__class__.__name__.lower().replace("processor", "")
                svg_data = self.get_cached_svg_or_render(
                    proc_name, match.group(0), self._render_schemdraw, code, node=node
                )
    ...
    def _render_schemdraw(self, code: str) -> bytes:
        ...
        with self._MATPLOTLIB_RENDER_LOCK:
            ...
            # Explicitly close all figures to prevent warnings and leaks.
            try:
                plt.close("all")
            except Exception:
                pass
        ...
        if viewbox_match:
            ...
            if len(parts) == 4:
                ...
                svg_str = svg_str.replace(
                    f'viewBox="{viewbox}"', f'viewBox="{new_viewbox}"'
                )
