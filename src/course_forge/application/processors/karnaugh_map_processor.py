...
class KarnaughMapProcessor(SVGProcessorBase):
        ...
        for match in matches:
            ...
            try:
                ...
                svg_data = self.get_cached_svg_or_render(
                    "karnaugh_map", match.group(0), render_kmap, node=node
                )
    ...
    def _parse_config(self, code: str) -> dict:
        ...
        if raw_values["raw_outputs"]:
            raise ValueError(
                "'outputs' parameter is deprecated. Please use 'grid' block instead."
            )
        ...
        # Parse Grid
        config["truthtable"] = self._parse_grid_to_truthtable(
            num_vars, raw_values["grid_lines"]
        )
    ...
    def _parse_grid_to_truthtable(
        self, num_vars: int, grid_lines: list[str]
    ) -> list[tuple[str, str]]:
        ...
        if len(grid_lines) != len(rows_gray):
            raise ValueError(
                f"Grid must have {len(rows_gray)} rows for {num_vars} variables"
            )
    ...
    def _render_kmap(self, config: dict) -> bytes:
        with self._MATPLOTLIB_RENDER_LOCK:
            ...
            d.add(
                logic.Kmap(
                    names=config["schemdraw_names"],
                    truthtable=config["truthtable"],
                    **kwargs,
                )
            )
